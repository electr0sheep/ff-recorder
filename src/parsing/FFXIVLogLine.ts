 ...
 */
export default class FFXIVLogLine {
  ...
  constructor(public original: string) {
    // console.debug('[FFXIVLogLine] ', this.original);
    ...
    this.timestamp = this.original.split('|')[1];

    // console.log('[FFXIVLogLine] Type: ', this.messageType);
    // console.log('[FFXIVLogLine] Timestamp: ', this.timestamp);
  ...
  date(): Date {
    ...
    if (timeParts.length >= 7) {
      ...
      const [year, month, day, hours, mins, secs, ms] = timeParts.map((v) =>
        parseInt(v, 10),
      );

      const milliseconds = ms / 10000;
