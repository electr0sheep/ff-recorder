...
export default abstract class FFXIVGenericLogHandler {
  ...
  protected async handleEncounterStartLine(
    ...
    console.debug(
      '[FFXIVGenericLogHandler] Handling ENCOUNTER_START line:',
      line,
    );

    if (FFXIVGenericLogHandler.activity) {
      console.warn('[FFXIVGenericLogHandler] Activity already in progress');
      return;
    }
    ...
    if (!isRecognisedDifficulty) {
      throw new Error(
        `[FFXIVGenericLogHandler] Unknown difficulty ID: ${difficultyID}`,
      );
    }
    ...
    if (!isRaidEncounter) {
      console.debug(
        '[FFXIVGenericLogHandler] Not a raid encounter, do nothing',
      );
      return;
    }
  ...
  protected async handleEncounterEndLine(line: FFXIVLogLine) {
    console.debug(
      '[FFXIVGenericLogHandler] Handling ENCOUNTER_END line:',
      line,
    );

    if (!FFXIVGenericLogHandler.activity) {
      console.info(
        '[FFXIVGenericLogHandler] Encounter stop with no active encounter',
      );
      return;
    }
    ...
    if (result) {
      ...
      FFXIVGenericLogHandler.activity.overrun = overrun;
    }

    FFXIVGenericLogHandler.activity.end(line.date(), result);
    await FFXIVGenericLogHandler.endActivity();
  }

  protected handleUnitDiedLine(line: FFXIVLogLine): void {
    if (!FFXIVGenericLogHandler.activity) {
      return;
    }
    ...
    const playerSpecId =
      FFXIVGenericLogHandler.activity.getCombatant(playerGUID)?.specID ?? 0;
    ...
    const activityStartDate = FFXIVGenericLogHandler.activity.startDate.getTime() / 1000;
    let relativeTime = deathDate - activityStartDate;

    if (relativeTime < 0) {
      console.error(
        '[FFXIVGenericLogHandler] Tried to set timestamp to',
        relativeTime,
      );
      relativeTime = 0;
    }
    ...
    FFXIVGenericLogHandler.activity.addDeath(playerDeath);
  }
